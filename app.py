...
def api_get_json(path: str, params: dict | None = None):
    ...
    try:
        ...
        if path == "/tmdb/search":
            data = main.tmdb_search(query=params.get("query"), page=params.get("page", 1))
            return data, None
            
        if path == "/home":
            data = main.home(category=params.get("category", "popular"), limit=params.get("limit", 24))
        ...
        if path.startswith("/movie/id/"):
            ...
            data = main.movie_details_route(tmdb_id)
            return data.model_dump(), None
            
        if path == "/movie/search":
            data = main.search_bundle(
